...
func pushGitOpsDirAndOpenPr(promotionName, destinationDir, printName string) error {
	...
	basePath := filepath.Join(destinationDir, appFolderName, "base")
	...
	containsNewFiles := gitChangesUpdateExistingFiles(appFolderName, worktree)
	// FIXME: remote next condition
	if containsNewFiles {
		fmt.Println(printName + " contains new files")
	}
	...
	// Git add the app folder
	//err = worktree.AddGlob(appFolderName + "/*")
	//if err != nil {
	//	return fmt.Errorf("failed to git add %s: %w", appFolderName, err)
	//}
	//
	//// Commit git changes with appropriate message
	//var commitMsg string
	//if containsNewFiles {
	//	commitMsg = fmt.Sprintf("chore: add GitOps base overlay for integration%s\n\nGenerated by Camel K promote command", promotionName)
	//} else {
	//	commitMsg = fmt.Sprintf("chore: update GitOps base overlay for integration%s\n\nGenerated by Camel K promote command", promotionName)
	//}
	//
	//commit, err := worktree.Commit(commitMsg, &git.CommitOptions{
	//	Author: &object.Signature{
	//		Name:  "Camel K",
	//		Email: "camel-k@apache.org",
	//		When:  time.Now(),
	//	},
	//})
	//if err != nil {
	...
	// determine whether this is "adding new file" or "updating new file" based on git changes and remember it for git message
	// git add "base" directory
	// commit git changes
...
func gitChangesUpdateExistingFiles(appFolderName string, worktree *git.Worktree) bool {
	status, err := worktree.Status()
	if err != nil {
		// not important enough to make any noise
		return false
	}

	for file, fileStatus := range status {
		// FIXME: this needs to be rewritten, probably use basePath and any changes there are simply our folders
		if strings.HasPrefix(file, appFolderName+"/") && fileStatus.Worktree == git.Untracked {
			return true
		}
	}
